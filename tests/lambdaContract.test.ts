...
/**
 * The API Lambda's handler is inline in infra/cloudformation.yaml — it can't import
...
describe("the inline Lambda's op contract", () => {
  const template = readFileSync(new URL("../infra/cloudformation.yaml", import.meta.url), "utf-8");

  it("dispatches exactly the op types the worker applies", () => {
    const literal = /const TARGET_KEY = \{([\s\S]*?)\};/.exec(template);
    expect(literal, "TARGET_KEY moved or changed shape in infra/cloudformation.yaml").not.toBeNull();
