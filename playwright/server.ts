...
function resultFor(op: QueuedOp): OpResult {
  ...
  if (op.type === "alert.remove") {
    return { id: op.id, type: op.type, symbol: "AA", alertId: op.target?.alertId ?? null, ok: true, message: `Removed static alert ${op.target?.alertId}.`, appliedAt };
  }
